...
use LegacyApi\Form\Api\Product\ProductImageModification;
...
class ImageController extends BaseApiController
{
    ...
    public function listAction($entityId)
    {
        ...
        return new JsonResponse($this->loopProvider->getLoopResults(new Image(), $params));
    }
    ...
    public function getImageAction($entityId, $imageId)
    {
        ...
        $results = new JsonResponse($this->loopProvider->getLoopResults(new Image(), $params));
    ...
    public function createImageAction($entityId, EventDispatcherInterface $dispatcher)
    {
        $request = $this->getRequest();
        ...
        foreach ($files as $file) {
            $errors = $this->processImage($dispatcher, $fileController, $file, $entityId, $entity, $config);
        }

        if (!empty($errors)) {
            $response = new JsonResponse($errors, 500);
        } else {
    ...
    public function updateImageAction($entityId, $imageId)
    {
        ...
        $baseForm = $this->createForm(ProductImageModification::getName(), FormType::class, [], array(
        ...
        if (! $baseForm->getForm()->isSubmitted()) {
            $baseForm->getForm()->submit($request->request->all());
        }
    ...
    protected function processImage(EventDispatcherInterface $dispatcher, FileController $fileController, $file, $entityId, $entity, array $config)
    {
        try {
    ...
    public function deleteImageAction($entityId, $imageId, EventDispatcherInterface $dispatcher)
    {
        $request = $this->getRequest();
        ...
        try {
            ...
            $dispatcher->dispatch($fileDeleteEvent, TheliaEvents::IMAGE_DELETE);
