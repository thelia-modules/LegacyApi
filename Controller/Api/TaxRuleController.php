...
use Thelia\Core\Form\Type\TaxRuleType;
...
class TaxRuleController extends AbstractCrudApiController
{
    ...
    {
        return $this->createForm(EmptyForm::getName(), TaxRuleType::class, $data);
    }
    ...
    {
        return $this->createForm(EmptyForm::getName(), TaxRuleType::class, $data, array(
    ...
    public function hydrateUpdateForm(FormEvent $event)
    {
        ...
        foreach ($persistentI18n["i18n"] as $locale => $value) {
            if (! isset($data["i18n"][$locale])) {
                $data["i18n"][$locale] = $value;
                continue;
            }
